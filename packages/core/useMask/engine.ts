...
/**
 * One entry of the hook's undo/redo history: the raw value plus the caret
 * offset to restore alongside it. The caret is the raw `selectionStart` read
 * off the live input at push time, so restoring it replays the position the
 * user was editing at rather than an end-of-value default.
 */
...
/**
 * Upper bound on the undo stack. Reaching it drops the oldest entry
 * (`Array.prototype.shift`), so the newest 100 edits stay undoable.
 */
...
/**
 * Resolve the placeholder written into the un-filled slot at `index`, where
 * `index` is the **position of the slot in the mask**, not a count of the slots
 * filled so far. `getSlotChar` is only ever called for token slots, and
 * `buildDisplayValue` passes the absolute slot index, so with a multi-character
 * `slotChar` the placeholder is read at that same slot position and literals
 * keep their own positions: `slotChar: 'DD/MM/YYYY'` on the mask `'99/99/9999'`
 * shows `'DD/MM/YYYY'` because the hint's own separators line up with the
 * mask's, while `(99) 99` with `'DD/MM'` shows `'(D/) __'` — the hint's `/` sits
 * at index 2, which the mask spends on its own `)` literal and never asks for,
 * and the two `9`s after it read `D` and `M`. A placeholder string shorter than
 * the mask falls back to `'_'`.
 *
 * `null`, `''` and `undefined` all disable placeholders and return `''`. That
 * is what stops `buildDisplayValue`: when the placeholders are disabled it
 * emits every literal up to the **first empty token slot** and then returns, so
 * `buildDisplayValue('', PHONE, null, true)` is `'('` for a mask with a leading
 * literal. A mask whose first slot is a token has no literal to emit first and
 * disables to `''`.
 */
 ...
 * none. It indexes `slots[i].type` without the guard its sibling has, so
 * `from >= slots.length` **throws** — even `([], 0)` does. Callers must pass an
 ...
 * map.
 *
 * `separate` is returned but never read by the engine or the hook: upstream
 * accepts the option and does nothing with it, and this port preserves that.
 */
