 ...
 * focus with `window.onfocus` and `window.onblur`. Focus state as a plain boolean — `true` on the
 * window `focus` event, `false` on `blur`.
 *
 * React divergences:
 * - the `focus`/`blur` listeners live in a self-contained `useEffect`;
 * - the initial `document.hasFocus()` sync happens in the mount effect instead of during setup, so
 * SSR renders the `false` default without touching `window.document` (matching upstream's no-window
 * value), and a falsy `window` — including a JS-passed `null` — disables tracking entirely,
 * mirroring upstream's `if (!window) return false`.
 *
 * @example
