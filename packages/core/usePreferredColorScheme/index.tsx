 ...
 * `useMediaQuery('(prefers-color-scheme: light)')` and `useMediaQuery('(prefers-color-scheme:
 * dark)')` and maps the two matched refs through `computed` to a string. Reactive
 ...
 * - the initial `matches` sync happens in an isomorphic layout effect — on the client it runs
 * before paint, so the very first frame already carries the real preference (no `'no-preference'`
 * flash), while SSR renders the `'no-preference'` default without touching `window` (, where both
