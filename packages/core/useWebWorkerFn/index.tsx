...
/**
 * Concatenates the dependencies into a comma separated string, used as the argument of the worker's
 * `importScripts` call. Local dependencies are stringified and hoisted into the worker script as
 * `const` bindings.
 *
...
/**
 * Returns the `onmessage` handler that runs inside the worker: it unpacks the posted arguments,
 * applies `userFunc`, and posts back `['SUCCESS', result]` / `['ERROR', error]` tuples.
 *
...
/**
 * Converts `fn` (plus its dependencies) into the source of a worker script and returns a blob URL
 * for it.
 *
 ...
 * function inside a dedicated Web Worker spawned from a blob URL, so the UI is not blocked. The
 * worker code is built by stringifying `fn` and the helper `lib/` functions (`createWorkerBlobUrl`,
 * `depsParser`, `jobRunner`), which are inlined below.
 *
 * React divergences:
 * - a worker is spawned per `workerFn()` call and terminated when the promise settles or
 * `workerTerminate()` runs, and the mount `useEffect` cleanup terminates any still-running worker
 * on unmount — including the `workerStatus` being reset to `PENDING`;
