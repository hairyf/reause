 ...
 * element's measured height, and **`0`** for a non-positive or non-numeric
 * height — the `typeof height === 'string'` half is what makes an unmeasurable
 * element (`getElementHeight` returns the string `'auto'`) settle instantly
 * instead of transitioning over a bogus duration.
 */
...
/**
 * Loose equality over a dependency list — same length, every entry `Object.is`.
 * Upstream's `dependenciesChanged` helper from
...
/**
 * Inlined from upstream's `source/mantine/packages/@mantine/hooks/src/use-did-update/`
 * — private on purpose.
 *
 * Built on the sibling `useUpdateEffect` from `@reause/shared`, as this port
 * requires: `useUpdateEffect` already owns the "skip the mount render" half, so
 * this adds only the dependency half rather than a third update-only primitive.
 * Upstream's own `useDidUpdate` is passed here with a dependency list by every
 * mantine hook in this directory, and that is the supported shape.
 *
 * Two behaviours are kept from upstream: a dependency list that did not change
 * (by `Object.is`, per entry) does not run the callback, and with a *changed*
 * list the callback runs with the previous list already recorded.
 *
 * One documented divergence, unreachable from `useCollapse`: upstream lets a
 * callback with **no** dependency list run on every commit, while `useUpdateEffect`
 * forwards no list to React's array identity check, which then never re-runs.
 * Pass a list.
 */
 ...
 * inlined privately (this repo has no existing `mergeRefs` equivalent; see the
 * hook doc below). Object refs get `current`, function refs get invoked, and
 * the cleanup half is React 19 aware: when any underlying callback ref returns
 * a cleanup, the merged callback returns one too and React calls it instead of
 * calling the refs again with `null`.
 */
