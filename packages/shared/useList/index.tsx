...
/**
 * Resolve a value-or-action into its value, mirroring react-use's
 * `resolveHookState` (`source/react-use/src/misc/hookState.ts`) **including its
 * arity rule**: when the action is a function, react-use calls it with the
 * current state only if it declares a parameter and with no argument otherwise
 * (`nextState.length ? nextState(currentState) : nextState()`). The issue's
 * mapping note suggests reusing reause's `SetStateAction`-style resolution
 * instead; that form always passes the previous value to a function action, so
 * it would silently change what a zero-arity action observes through
 * `arguments` — the pin wins, and the difference is measured by the differential
 * probe recorded in the PR. Both accepted forms are unaffected: a `() => next`
 * factory ignores the argument either way, and a `prev => next` updater still
 * receives the current list.
 *
 * Note the init action is typed `() => S` only, so a resolve without
 * `currentState` can never reach the parameterised branch through the public
 * API; react-use's own overload accepts the wider union there, and this port
 * keeps the shared implementation so the behaviour stays identical.
 */
