 ...
 * Mapping:. Two surface differences only:
 ...
 *
 * The returned function **is a hook** — it calls `useMemo` — so call the factory once, at module
 * scope, and bind the result to a `useXxx` name; only then does React's Rules of Hooks accept it at
 * a call site:
 *
 * ```ts
 ...
 * ```
 *
 * Dependencies are the raw `args`, compared by **reference equality**, exactly like upstream. The
 * memoised body re-runs whenever an argument is a new reference, even when it is structurally
 * equal: pass a fresh object, array or callback on every render and `createMemo` degenerates into
 * an unmemoised call. Callers must pass **already-stable arguments** (a state value, a module
 * constant, a `useMemo`d object). Deliberately *not* "fixed" by cloning or deep-comparing the
 * arguments here — that would silently diverge from upstream's dependency semantics.
 *
 * Not to be confused with `useMemoize` (@reause/core), the VueUse port: that one is a **persistent
 * cache keyed by the arguments**, shared across calls and components, while `createMemo` is a
 * render-scoped `useMemo` wrapper — the value is recomputed per mounting component and nothing is
 * cached after it unmounts.
 *
