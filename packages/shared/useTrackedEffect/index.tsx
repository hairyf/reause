 ...
 * `useEffect` whose callback additionally receives **which** dependencies moved, as an ascending
 * list of indexes into the dependency array, so one effect that refetches several things can react
 * only to the one that actually changed. Upstream default-exports the hook; reause exports it as a
 * named export, the convention for these mirrors.
 *
 * **The changed-index list is the contract, and these are the pin's exact semantics, measured
 * rather than assumed:**
 *
 ...
 *
 * The effect is called as `effect(changes, previousDeps, currentDeps)`: the second argument is the
 * deps of the previous run (`undefined` on the first), the third the deps of this run. The stored
 * deps are assigned **before** the effect is invoked; the observable consequence under
 * `<StrictMode>` is that the double-invoked mount effect reports `[0, 1, …]` on its first
 * invocation and `[]` on its second, because the ref already holds the deps by then.
 *
 * The effect's return value is forwarded unchanged as the cleanup, so `void | (() => void)` behaves
 * exactly as it does for `useEffect`.
 *
 * `deps` is typed `[...T]` — a tuple — so the hook infers the caller's literal dependency list
 * positionally and the indexes in `changes` line up with it.
 *
 * Not `useUpdateEffect` (#940): that hook **skips** the mount and has no `changes` argument, while
 * this one runs on mount and reports every index there.
 *
 * SSR-safe: nothing touches `window`/`document`, at import time or on first render — the render
 * phase is `useRef` alone and all work happens in the passive effect.
 *
 * @example
