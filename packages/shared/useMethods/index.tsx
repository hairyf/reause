...
/**
 * The action a wrapped method dispatches: the method's name in `type`, and the
 * wrapper's **whole argument list** in `payload`.
 *
 * `payload` is the rest-args array, not a single argument — the reducer spreads
 * it back into the method (`...action.payload`), which is what makes
 * `increment(1, 2)` reach `increment` as two arguments rather than one array.
 * Both `any`s are the pin's, kept verbatim; this repo's eslint config carries no
 * `no-explicit-any` rule. The pin declares this shape as a `type` alias, which
 * the enforced `ts/consistent-type-definitions` rule rejects for an object
 * literal; it is an `interface` here, the same form `createReducer` uses for its
 * `Store`.
 */
interface Action {
  type: string
