...
/**
 * Build a `useReducer`-shaped hook around a Redux-style middleware chain — React port of
 * react-use's `createReducer`.
 *
 * Map from react-use `createReducer`
 * (source/react-use/src/factory/createReducer.ts)
 * Mapping:. The factory closes over `middlewares` and composes them once; the function it returns
 * **is the hook** — `(reducer, initialState, initializer?) => [state, dispatch]` — with
 * `initializer` in upstream's third parameter position and defaulting to the identity function, and
 * with the `[state, dispatch]` tuple shape unchanged. Three deliberate surface differences only:
 ...
 * - `Dispatch` / `Store` / `Middleware` stay module-private, so the only new export is
 * `createReducer`. To name the middleware type at a call site, derive it from a bound hook
 * (`Parameters<typeof useMyReducer>[0]`) or write the middleware inline, where the factory's rest
 * parameter types it contextually.
 *
 * **The ref + `useState` pair is upstream's design and is kept on purpose.** The state lives in
 * `ref.current`; `useState` exists only to schedule the re-render. `dispatch` writes the ref
 * **synchronously** and then mirrors that value into React state, which is what buys the three
 * guarantees this hook is chosen for:
 ...
 * `MutableRefObject<Dispatch<Action>>`; that type is gone in React 19, so the container is written
 * explicitly as `{ current: Dispatch<Action> }` (docs/subagent-execution.md §3.2 forbids `as
 * MutableRefObject<T>`). React 19's `useRef` returns exactly that mutable shape, so the annotation
 * needs no assertion.
 *
 * **`dispatch` returns the action it was given.** Middleware relies on it — a middleware that calls
 * `store.dispatch` gets the value back through the chain, as long as each middleware returns
 * `next(action)` — while the public `Dispatch<Action>` type says `void`, so TypeScript hides it.
 * Worth recording precisely, because the issue's premise that the port "needs the same cast
 * upstream uses" does not survive the pin: `source/react-use/src/factory/createReducer.ts` contains
 * **no `as` at all** (67 lines, zero assertions). The only type-level convergence in upstream is
 * the `MutableRefObject<Dispatch<Action>>` annotation, and because `action` is annotated `Action`
 * here (strict mode forbids upstream's implicit `any`), `useCallback` infers `(action: Action) =>
 * Action`, which is structurally assignable to the `void`-returning `Dispatch<Action>` — so no
 * assertion is needed, and adding one would be a gratuitous `as` that the `dispatchRef` annotation
 * below already makes redundant.
 *
 * **Re-composition is keyed on `dispatch`, not on the middleware array.** `useUpdateEffect`
 * re-composes the chain whenever its `dispatch` dependency changes, and `dispatch` is
 * `useCallback(..., [reducer])` — so the trigger is the **reducer identity**, exactly as the pin
 * and issue #931 describe ("the middleware chain is re-composed when `dispatch` changes"). A fresh
 * middleware array, or a brand-new middleware function, with the *same* reducer reference does
 * **not** re-compose. Because the re-composition runs in an effect that writes the ref, the render
 * that observed the change still returns the previous chain and the next render returns the
 * re-composed one. The middleware list is captured when the factory runs, so a caller that needs a
 * varying list must call `createReducer(...)` during render (binding its result to a `useXxx` name)
 * and change the reducer identity along with it.
 *
 * **`useUpdateEffect` is imported, not inlined.** The sibling port is merged in
 * `packages/shared/useUpdateEffect` and re-exported from the `@reause/shared` barrel, so
 * re-composition is done by the very hook upstream imports — `../useUpdateEffect`, the relative
 * form every shared hook uses so a source file never imports its own package by name
 * (eslint.config.js's self-import guard).
 *
 * Out of scope on purpose: upstream's `createReducerContext` and `createStateContext` are separate
 * factories and are not ported here.
 *
 * The returned function **is a hook** — it calls `useRef`, `useState`, `useCallback` and
 * `useUpdateEffect` — so bind the factory's result to a `useXxx` name and call that inside a
 ...
 *
 * const useCounterReducer = createReducer<Action, State>( // `store` / `next` / `action` are
 * contextually typed by the factory store => next => (action) => { const result = next(action)
 * console.log('count', store.getState().count) // already the new state return result }, )
 *
 * function Counter() { const [state, dispatch] = useCounterReducer( (state, action) => action.type
 * === 'increment' ? { count: state.count + 1 }: { count: action.payload }, { count: 0 }, )
 *
 * return (
 ...
 * ```
 *
 * Upstream mapping files: `source/react-use/src/factory/createReducer.ts` (67 LOC) and
 * `source/react-use/docs/createReducer.md`. There is **no upstream test file** to mirror — the
 * pinned tree ships no test suite at all
