 ...
 * Mapping:, not a tuple and not a wrapper, so `map instanceof Map` holds at the call site and
 * `size`, `get`, `has` and iteration are the real `Map` semantics. Only `set`, `delete` and `clear`
 * are replaced by own methods; each one applies the pristine `Map.prototype` method through
 * `proto.*.apply(map, args)` and then asks `@reause/shared`'s `useUpdate` for a re-render (upstream
 * calls react-hookz's own `useRerender`, which this port reuses rather than duplicating — see
 * `docs/orchestration.md`). The patch runs inside the lazy `useRef` init branch, so the `Map` is
 * constructed exactly once per component and keeps a stable identity for the component's lifetime;
 * `entries` is read **only** by that first construction — a later, changed `entries` argument is
 * ignored, which is what makes the stable identity possible. The patch calls the pristine prototype
 * method rather than `map.set`, so it cannot recurse into itself.
 *
 * The returned `set` returns the patched `Map` explicitly (`return map`), as upstream does;
 * `delete` returns the native boolean and `clear` the native `undefined` (upstream returns neither
 * explicitly, and this port mirrors that).
 *
 * A different shape from react-use's `useMap`, which is deliberately **not** ported here: react-use
 * returns a `[map, utils]` tuple over a plain object (`T extends object`) whose `utils` are `{ get,
 * set, setAll, remove, reset }`, not a `Map` at all. Only react-hookz's shape is mirrored, so the
 ...
 * re-render runs the mutation again — an unguarded in-render `map.set(...)` loops until React bails
 ...
 * SSR-safe. Upstream ships `useMap` as a named export too (every react-hookz hook is one directory,
 * `src/useMap/index.ts`); reause exports it by name from `@reause/shared`.
 *
