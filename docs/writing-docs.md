...
| 代码块 twoslash（构建期类型检查 → 悬停卡片）                                    | 同上（`ts`/`tsx`/`typescript` 块自动补 `twoslash` meta，并注入 `// @include: imports`）              | 块自身没写 `no-twoslash`                            |
...
- 围栏**不需要**写 `twoslash`：`ts` / `tsx` / `typescript` 块由 `markdownTransform.ts` 自动补上 meta，并注入 `// @include: imports`（注入内容被 `// ---cut-*---` 裁掉，读者看不到）。注入的是 `@reause/*` 各包导出的 Hook 名（由 registry 生成）+ 常用 React Hook，所以**续写型示例**（不写 import 的 `const { x, y } = useMouse()`）也能悬停出真实签名。
- 注入只覆盖 Hook 名；示例里引用**类型**（`UseXxxOptions` 等）时仍要自己写 `import type`。
- 关掉单块的类型检查（伪代码、故意不成立的写法）就在 meta 写 `no-twoslash`；行高亮照常可用（meta 写 `{5}`，会被补成 `{5} twoslash`）。`js` / `jsx` 块默认不参与，需要时手写 `twoslash`。
